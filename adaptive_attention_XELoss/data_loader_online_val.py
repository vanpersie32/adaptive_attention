...
class DataLoader(object):
    ...
    '''
    def __init__(self,opt,phase,val_file):
        ...
        assert phase == 'val'
        # for online test, generate cation for validation sets
        # first read validation information
        self.val_set = val_file 
        assert self.val_set
        with open(self.val_set) as f:
            val_info = json.load(f)
        val_image_id = [str(image['id']) for image in val_info['images']]
        # filter all image id and only save the validation set
        self.val_ix = [i for i,image_id in enumerate(image_ids) if str(image_id) in val_image_id]
        print('val set:{:d}'.format(len(self.val_ix)))
    ...
    def get_batch(self,batch_size):
        ...
        if self.phase == 'train':        
            ...
            return attributes, features, image_feature, input_seqs, target_seqs
        else:
