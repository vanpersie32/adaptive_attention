...
class LanguageModel(object):
    ...
    def build_inputs(self):
        ...
        if self.phase == 'train':
            ...
            attri = tf.placeholder(dtype = tf.float32,shape = [1,
                                                               self.opt.attr_size],
                                                      name='attributes')
            ...
            # expand the batch dim so that the batch dim of  attributes, objects feature , image features match that of input_seqs
            self.attri = tf.tile(attri,[batch_size,1])
            self.objects_features = tf.tile(objects_features,[batch_size,1,1])
            self.image_features = tf.tile(image_features,[batch_size,1])
    ...
    def build_model(self):
        ...
        def first_layer_attention(step,h,all_atten,all_inputs):
        ...
        def second_layer_attention(step,attri_context,obj_context,h,second_atten,reuse):
            with tf.variable_scope('second_att',reuse = reuse):
                ...
                obj_emb = slim.fully_connected(obj_linear,
                                               ...
                                               reuse = reuse)
                '''
                history_linear = slim.fully_connected(history_context,
                                                      mapped_size,
                                                      activation_fn=tf.nn.relu,
                                                      scope='history_linear',
                                                      reuse = reuse)
                
                history_emb = slim.fully_connected(history_linear,
                                                   mapped_size,
                                                   activation_fn=None,
                                                   scope='history_emb',
                                                   reuse = reuse)
        ...
        def attended_lstm(step,states,loss,first_atten,second_att,all_inputs):
            
            # build_attention
            c,h = tf.split(states,2,axis=1)
            new_outputs,new_first_att = first_layer_attention(step,h,first_atten,all_inputs)
            ...
            if self.phase == 'train':
                ...
                return step+1,new_states,new_loss,new_first_att,new_second_att,all_inputs

            # calculate probability over vocabulary
            else:
                tf.nn.softmax(logits,name='prob')
                return 0, new_states, 0.0, new_first_att,new_second_att,all_inputs
        ...
        if self.phase == 'train':
            for i in xrange(self.opt.seq_length+1):
                ...
                reuse = reuse or (i!=0)
                _,states, loss,first_atten,second_atten, all_inputs = attended_lstm(tf.constant(i),states,loss,first_atten,second_atten,all_inputs)
            ...
            self.top20_weights = self.attr_atten
            ...
            all_inputs = [self.attri,self.objects_features,seq_embeddings]            
            _,new_states,_,first_atten,second_atten,_= attended_lstm(step,state_feed,tf.constant(0,dtype = tf.float32),first_atten,second_atten,all_inputs)
            self.top20_weights = first_atten[0].read(step)
